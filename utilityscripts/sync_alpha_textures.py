...
def prompt_assets_root() -> Path | None:
    ...
    print("=" * 60)
    response = input(
        "Enter path to your cobblemon-assets clone\n"
...
def build_folder_map() -> dict[str, Path]:
    """Build a lookup of normalized_species_name -> folder path from TEXTURES_DIR."""
    folder_map = {}
    for folder in TEXTURES_DIR.iterdir():
        ...
        if len(parts) == 2:
            # Store both the raw name and a normalized version (no hyphens/underscores)
            # so ho_oh -> hooh, mr_mime -> mrmime etc. can match 0250_hooh, 0122_mrmime
            raw_name = parts[1]
            folder_map[raw_name] = folder
            normalized = raw_name.replace("_", "").replace("-", "")
            if normalized != raw_name:
                folder_map[normalized] = folder
    return folder_map


def normalize(name: str) -> str:
    """Strip hyphens and underscores for loose matching."""
...
def find_dest_folder(filename_stem: str, folder_map: dict[str, Path]) -> Path | None:
    ...
    Tries the full species name first, then progressively strips trailing
    segments until a match is found. Each attempt is tried both raw and
    normalized (hyphens/underscores stripped) to handle cases like:
      ho_oh_alpha       -> try ho_oh, then hooh -> matches 0250_hooh
      mr_mime_alpha     -> try mr_mime, then mrmime -> matches 0122_mrmime
      porygon-z_alpha   -> try porygon-z, then porygonz -> matches NNNN_porygonz
      zorua_hisuian_alpha -> try zorua_hisuian, then zorua -> matches 0570_zorua
    """
...
def main():
    ...
    if not TEXTURES_DIR.exists():
        ...
        print("Is this script in the cobblemon/utilityscripts/ directory?")
        sys.exit(1)
