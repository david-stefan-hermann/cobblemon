...
class TMPartySlotWidget(
    ...
    override fun renderWidget(context: net.minecraft.client.gui.GuiGraphics, mouseX: Int, mouseY: Int, delta: Float) {
        ...
        if (pokemon != null) {
            val resource = if (teachable !== null) (
                    if (teachable as Boolean) slotLearnableResource
                    else slotdisabledResource
                )
                else slotResource
            ...
            teachable?.let {
                drawScaledText(
                    context = context,
                    text = lang("ui.tm_machine.${if (it) "can" else "cannot"}_learn"),
                    x = x + (width / 2F),
                    y = y + 26,
                    centered = true,
                    scale = HALF_SCALE
