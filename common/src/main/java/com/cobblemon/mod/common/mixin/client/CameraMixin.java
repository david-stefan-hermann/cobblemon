...
/**
 * port/26.2: Camera.setup(level, entity, detached, thirdPersonReverse, partialTick) became
 * alignWithEntity(partialTicks) - level and entity are set beforehand in update, the level field is a
 * Level, and the mirrored-camera flag is read from the options inline instead of being passed in.
 */
@Mixin(Camera.class)
public abstract class CameraMixin implements CameraDuck {
    ...
    public void cobblemon$positionCamera(Camera instance, double x, double y, double z, Operation<Void> original) {
        ...
        if(vehicle instanceof Rideable){
            boolean thirdPersonReverse = Minecraft.getInstance().options.getCameraType().isMirrored();
